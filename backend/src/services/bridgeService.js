...
class BridgeService {
  ...
  /**
   * MODIFIED: Adds a donor to a specific bridge.
   * Can be used when a donor accepts an invitation.
   */
  async addDonorToBridge(donorId, bridgeId) {
    try {
      await db.query(
        "INSERT INTO bridge_members(bridge_id, donor_id, position) VALUES($1, $2, (SELECT COALESCE(MAX(position), 0) + 1 FROM bridge_members WHERE bridge_id = $1)) ON CONFLICT DO NOTHING",
        [bridgeId, donorId]
      );
      console.log(`Successfully added donor ${donorId} to bridge ${bridgeId}.`);
      return { success: true, status: 'added_to_bridge' };
    } catch (error) {
      console.error('Error adding donor to bridge:', error);
      return { success: false, error };
  ...
  /**
   * Handles a general request from a donor to join a bridge via their phone number.
   * This is called by the AI router if a donor says "I want to join a bridge".
   */
  async addDonorToBridgeByPhone(donorPhone) {
    const sanitizedPhone = normalizePhoneNumber(donorPhone);
    try {
      const { rows: [donor] } = await db.query('SELECT id, name, city, blood_group FROM users WHERE phone = $1', [sanitizedPhone]);
      if (!donor) return "It looks like you're not registered yet. Please register first!";
     
      const { rows: [existingMember] } = await db.query('SELECT bridge_id FROM bridge_members WHERE donor_id = $1', [donor.id]);
      if (existingMember) return `Thank you, ${donor.name}! You are already a valued member of a Blood Bridge.`;
      
      const bridge = await this.findBestBridgeForDonor(donor.city, donor.blood_group);
      if (!bridge) return `Thank you for your interest, ${donor.name}! We don't have a Blood Bridge matching your profile right now, but we'll notify you when one becomes available.`;
      
      await this.addDonorToBridge(donor.id, bridge.id);
  ...
  /**
   * NEW: Finds top donors and sends them an invitation to join a new bridge.
   * This is called by the admin controller.
   */
  async inviteDonorsToBridge(bridgeId, city, bloodGroup, pincode) {
    try {
        ...
        const { rows: bestDonors } = await db.query(
            'SELECT donor_id, name, phone FROM find_donors_for_bridge($1, $2, $3, $4)',
            [city, bloodGroup, pincode, 8]
        );

        if (!bestDonors || bestDonors.length === 0) {
            console.warn(`No donors found to invite for bridge ${bridgeId}`);
            return { success: true, count: 0 };
        }

        const invitationMessage = `You are a top match to help a Thalassemia patient, *${bridge.patient_name}*, in your area. Would you be willing to join their dedicated "Blood Bridge" to provide regular, life-saving donations?`;
        
        for (const donor of bestDonors) {
            const buttons = [
                { id: `accept_bridge_invitation_${donor.donor_id}_${bridgeId}`, title: "Yes, I'll Help" },
                { id: `decline_bridge_invitation_${donor.donor_id}_${bridgeId}`, title: "Not at this time" }
            ];
            await whatsappService.sendInteractiveMessage(donor.phone, invitationMessage, buttons);
        }

        return { success: true, count: bestDonors.length };
    } catch (error) {
        console.error(`Error inviting donors to bridge ${bridgeId}:`, error);
        throw error;
    }
  }
}
